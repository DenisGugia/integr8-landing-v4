...
- Background: var(--dark) com backdrop-filter: blur(12px) — mantém legibilidade ao scrollar sobre conteúdo
- Sticky no topo (position: fixed)
- Esconde ao scrollar para baixo (>50px), reaparece ao scrollar para cima — implementado via IntersectionObserver ou scroll direction detection
...
**Atenção:** a sticky bar ocupa 60px do viewport. Em mobile, o conteúdo do footer não pode ficar escondido atrás dela. Adicionar `padding-bottom: 60px` no `<body>` apenas quando a barra está visível.

---

## SEÇÃO 1 — HERO MOBILE

**Decisão:** foto como fundo full-width com overlay de texto (imersivo).

### Comportamento mobile

O hero ocupa 100svh (small viewport height — usa svh para evitar o problema do endereço do Safari que muda o 100vh).
...
.hero {
  ...
  justify-content: flex-end; /* texto na metade inferior */
  overflow: hidden;
}

/* Foto do casal como background full-width */
.hero-photo {
  position: absolute;
  inset: 0;
  width: 100%;
  height: 100%;
  object-fit: cover;
  object-position: center top; /* foca no casal, não no fundo */
  z-index: 0;
}

/* Overlay gradiente — escurece a metade inferior para o texto */
.hero::after {
  content: '';
  position: absolute;
  inset: 0;
  background: linear-gradient(
    to bottom,
    rgba(0,0,0,0.1) 0%,    /* topo: foto visível */
    rgba(0,0,0,0.2) 35%,   /* meio: transição */
    rgba(0,0,0,0.75) 65%,  /* inferior: escurece para texto */
    rgba(0,0,0,0.90) 100%  /* base: quase opaco */
  );
  z-index: 1;
}

/* Conteúdo de texto — sobre o overlay */
.hero-content {
  position: relative;
  z-index: 2;
  padding: 0 24px 80px; /* 80px de bottom para não colidir com sticky bar */
}
```
...
Velocidade de animação: 45s em mobile (mais lento que desktop 60s) para compensar viewport menor — o texto passa em menos frames.
...
track.addEventListener('scroll', () => {
  ...
  const activeIndex = Math.round(track.scrollLeft / cardWidth / 2); // /2 pois 2 cards por "página"
...
@media (min-width: 1024px) {
  ...
  .phone-side:first-of-type { order: 1; }
  .phone-side:last-of-type { order: 3; }
