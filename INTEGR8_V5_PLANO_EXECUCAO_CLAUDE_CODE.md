...
- O domínio `www.integr8-hub.com` já está apontado para `DenisGugia/integr8-landing-v4` via Netlify auto-deploy
...
- Mudar para repo novo significa reapontar DNS, refazer Netlify, possivelmente rebuildar GTM
- Branch nova permite deploy preview no Netlify para testar antes de subir para produção
...
A partir daí, todo trabalho do Claude Code acontece nessa branch. O Netlify gerará deploy preview automaticamente em URL do tipo `v5-copy-overhaul--integr8-hub.netlify.app`. Quando aprovado, merge na main → deploy automático para produção.
...
| 7 | Revisão final: acessibilidade, performance, responsividade, A/B test setup | Claude Opus 4.6 ou 4.7 | Revisão crítica exige raciocínio profundo e olhar de design senior |
...
> "Estamos na branch `v5-copy-overhaul`. Antes de qualquer modificação no HTML, faça um inventário completo de: (a) estrutura atual de pastas, (b) todos os arquivos HTML existentes e qual deles é o ativo em produção via Netlify, (c) todos os assets dentro de `/IMAGENS/`, (d) qual é o arquivo principal que recebe deploy. Devolva esse inventário antes de qualquer outra ação."
...
4. **Lista de identificação (5 itens, ul.check-list):**
   ...
   Cada item:
   - Marcador `▶` em neon (já implementado)
...
- Esquerda:
  ...
  - Tagline abaixo: `© 2025 Integr8 · Denis Gugia · Todos os direitos reservados` (font 12px, muted)
...
A partir daí, siga as fases na ordem, com aprovação entre elas. Use Sonnet 4.6 nas fases 1 a 6 e Opus 4.6 ou 4.7 na fase 7 (revisão crítica final).
